...
class system_settings extends MY_Controller
{
    ...
    function index()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = array('site_name' => DEMO ? 'Stock Manager Advance' : $this->input->post('site_name'),
                ...
                'chg_points_minimum' => $this->input->post('chg_points_minimum'),
                'chg_points_maximum' => $this->input->post('chg_points_maximum'),
            ...
            if ($this->input->post('smtp_pass')) {
                $data['smtp_pass'] = $this->uc->encrypt($this->input->post('smtp_pass'));
            }
        }
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateSetting($data)) {
            ...
            $this->data['smtp_pass'] = $this->uc->decrypt($this->data['settings']->smtp_pass);
    ...
    function getCategories()
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("{$this->db->dbprefix('categories')}.id as id, {$this->db->dbprefix('categories')}.image, {$this->db->dbprefix('categories')}.code, {$this->db->dbprefix('categories')}.name, (CASE WHEN {$this->db->dbprefix('categories')}.no_points = 1 THEN '".lang('category_no_points_1')."' ELSE '".lang('category_no_points_0')."' END) as no_points", FALSE)
            ->from("categories")
    ...
    function getCustomerGroups()
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("id, name, percent, points_required, reset_points_percent, maintain_sales")
    ...
    function add_customer_group()
    {
        ...
        if ($this->form_validation->run() == true) {
            $data = array(
                ...
                'reset_points_percent' => $this->input->post('reset_points_percent'),
                'maintain_sales' => $this->input->post('maintain_sales'),
        ...
        if ($this->form_validation->run() == true && $this->settings_model->addCustomerGroup($data)) {
            ...
            // reorder level lại theo thứ tự dựa vào phần trăm (trừ nhóm khách lẻ id=1)
            $this->settings_model->reorderLevel();
    ...
    function size_group()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('size_group')));
        $meta = array('page_title' => lang('size_group'), 'bc' => $bc);
        $this->page_construct('settings/size_group', $meta, $this->data);
    }
    ...
    function getSizeGroup()
    {
        ...
        $this->datatables
            ...
            ->from("size_group")
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_size_group/$1') . "' class='tip' title='" . lang("edit_size_group") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_size_group") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_size_group/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
        //->unset_column('id');

        echo $this->datatables->generate();
    }

    function add_size_group()
    {

        $this->form_validation->set_rules('name', lang("size_group_name"), 'trim|is_unique[size_group.name]|required');

        if ($this->form_validation->run() == true) {
            $data = array('name' => $this->input->post('name'), 'description' => $this->input->post('description'));
            //$this->sma->print_arrays($data);
        } elseif ($this->input->post('add_size_group')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/size_group");
        }

        if ($this->form_validation->run() == true && $this->settings_model->addSizeGroup($data)) {
            $this->session->set_flashdata('message', lang("size_group_added"));
            admin_redirect("system_settings/size_group");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_size_group', $this->data);
        }
    }

    function edit_size_group($id = NULL)
    {

        $this->form_validation->set_rules('name', lang("size_group_name"), 'trim|required');
        $size_group = $this->settings_model->getSizeGroupByID($id);
        //$this->sma->print_arrays($size_group);
        if ($this->input->post('name') != $size_group->name) {
            $this->form_validation->set_rules('name', lang("size_group_name"), 'is_unique[size_group.name]');
        }
        $this->form_validation->set_rules('description', lang("size_group_description"), 'required|min_length[3]');
        ...
        } elseif ($this->input->post('edit_size_group')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/size_group");
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateSizeGroup($id, $data)) {
            $this->session->set_flashdata('message', lang("size_group_updated"));
            admin_redirect("system_settings/size_group");
        } else {
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['size_group'] = $size_group;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_size_group', $this->data);
        }
    }

    function delete_size_group($id = NULL)
    {
        if ($this->settings_model->deleteSizeGroup($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("size_group_deleted")));
        }
    }
    ...
    function promotion_list()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('promotion_list')));
        $meta = array('page_title' => lang('promotion_list'), 'bc' => $bc);
        $this->page_construct('settings/promotion_list', $meta, $this->data);
    }
    ...
    function getPromotionList()
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("id, name, description, DATE_FORMAT({$this->db->dbprefix('promotion_list')}.start_date, '%d/%m/%Y') as start_date, DATE_FORMAT({$this->db->dbprefix('promotion_list')}.end_date, '%d/%m/%Y') as end_date, (CASE WHEN active = 1 THEN '".lang('promotion_list_active_yes')."' ELSE '".lang('promotion_list_active_no')."' END) as active")
            ->from("promotion_list")
            //->add_column("Actions", "<div class=\"text-center\"></div>", "id");
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_promotion_list/$1') . "' class='tip' title='" . lang("edit_promotion_list") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_promotion_list") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_promotion_list/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
    ...
    function add_promotion_list()
    {

        $this->form_validation->set_rules('name', lang("promotion_list_name"), 'trim');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/promotion_list");
        }

        if ($this->form_validation->run() == true && $this->settings_model->addPromotionList($data)) {
            $this->session->set_flashdata('message', lang("promotion_list_added"));
            admin_redirect("system_settings/promotion_list");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));

            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_promotion_list', $this->data);
        }
    }

    function edit_promotion_list($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updatePromotionList($id, $data)) {
            $this->session->set_flashdata('message', lang("promotion_list_updated"));
            admin_redirect("system_settings/promotion_list");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));

            $this->data['promotion_list'] = $this->settings_model->getPromotionListByID($id);

            $this->data['id'] = $id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_promotion_list', $this->data);
        }
    }

    function delete_promotion_list($id = NULL)
    {
        if ($this->settings_model->deletePromotionList($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("promotion_list_deleted")));
        }
    }

    function promotion_list_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    foreach ($_POST['val'] as $id) {
                        $this->settings_model->deletePromotionList($id);
                    }
                    $this->session->set_flashdata('message', lang("promotion_list_deleted"));
                    redirect($_SERVER["HTTP_REFERER"]);
                }

            } else {
                $this->session->set_flashdata('error', lang("no_promotion_list_selected"));
                redirect($_SERVER["HTTP_REFERER"]);
            }
        } else {
            $this->session->set_flashdata('error', validation_errors());
    ...
    function product_description_list()
    {
        ...
        $meta = array('page_title' => lang('product_description_list'), 'bc' => $bc);
        $this->page_construct('settings/product_description_list', $meta, $this->data);
    }

    function getProductDescriptionList()
    {

        $this->load->library('datatables');
        $this->datatables
            ->select("id, name, description")
            ->from("product_description_list")
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_product_description/$1') . "' class='tip' title='" . lang("edit_product_description") . "' data-toggle='modal' data-target='#myModal'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_product_description") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_product_description/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
    ...
    function add_product_description()
    {
        ...
        $this->form_validation->set_rules('name', lang("product_description_name"), 'trim');
        ...
        } elseif ($this->input->post('add_product_description')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/product_description_list");
        }
        
        if ($this->form_validation->run() == true && $this->settings_model->addProductDescription($data)) {
            $this->session->set_flashdata('message', lang("product_description_added"));
            admin_redirect("system_settings/product_description_list");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));

            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_product_description', $this->data);
        }
    }

    function edit_product_description($id = NULL)
    {
        ...
        } elseif ($this->input->post('edit_product_description')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/product_description_list");
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateProductDescription($id, $data)) {
            $this->session->set_flashdata('message', lang("product_description_updated"));
            admin_redirect("system_settings/product_description_list");
        } else {
            $this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));

            $this->data['product_description_list'] = $this->settings_model->getProductDescriptionByID($id);

            $this->data['id'] = $id;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_product_description', $this->data);
        }
    }

    function delete_product_description($id = NULL)
    {
        if ($this->settings_model->deleteProductDescription($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("product_description_deleted")));
        }
    }

    function product_description_actions()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    foreach ($_POST['val'] as $id) {
                        $this->settings_model->deleteProductDescription($id);
                    }
                    $this->session->set_flashdata('message', lang("product_description_deleted"));
                    redirect($_SERVER["HTTP_REFERER"]);
                }

            } else {
                $this->session->set_flashdata('error', lang("no_product_description_selected"));
                redirect($_SERVER["HTTP_REFERER"]);
            }
        } else {
            $this->session->set_flashdata('error', validation_errors());
    ...
    function checkExistPercent($percent = NULL)
    {
        echo $this->settings_model->getCustomerGroupByPercent($percent);
    }
