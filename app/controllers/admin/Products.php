...
class Products extends MY_Controller
{
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            ...
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name, {$this->db->dbprefix('categories')}.name as cname, {$this->db->dbprefix('companies')}.company as supplier, cost as cost, price as price, COALESCE(quantity, 0) as quantity, '' as rack", FALSE)
                ->from('products')
