...
class Sales extends MY_Controller
{
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $grand_total_extra = $this->sma->formatDecimal(($total_extra - $order_discount), 4);
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $total_tax = $this->sma->formatDecimal(($product_tax + $order_tax), 4);
            $grand_total = $this->sma->formatDecimal(($total + $total_tax + $this->sma->formatDecimal($shipping) - $order_discount - $returns), 4);
            $grand_total_extra = $this->sma->formatDecimal(($total_extra - $order_discount), 4);
            $data = array('date' => $date,
                'reference_no' => $reference,
                'customer_id' => $customer_id,
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    if ($this->input->post('discount') && $item_discount == 0) {
                        $order_discount_id = $this->input->post('order_discount');
                        $opos = strpos($order_discount_id, $percentage);
                        if ($opos !== false) {
                            $ods = explode("%", $order_discount_id);
...
//                    }
        ...
        if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment, $si_return, $extra_data)) {
            ...
            foreach ($inv_items as $item) {
                if ($item->item_discount > 0) {
                    ...
                    $total_return_discount += $this->sma->formatDecimal($item->item_discount);
                }
                $row = $this->site->getProductByID($item->product_id);
                if (!$row) {
                    $row = json_decode('{}');
                ...
                $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'no_points' => $category->no_points, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'units' => $units, 'tax_rate' => $tax_rate, 'options' => $options);
                $c++;
            }
            $this->data['inv_items'] = json_encode($pr);
            $this->data['total_return_discount'] = $total_return_discount;
            $this->data['id'] = $id;
    ...
    public function suggestions()
    {
        ...
        $rows = $this->sales_model->getProductNames($sr, $warehouse_id, 14);
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                //qtthuan: khach vip khong tru % truc tiep tren sp
    ...
    public function get_return_sale($no)
    {
        if ($data = $this->sales_model->getReturnByReferenceNo($no)) {
