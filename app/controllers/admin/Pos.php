...
class Pos extends MY_Controller
{
    ...
    public function getSales($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, DATE_FORMAT(" . $this->db->dbprefix('sales') . ".date, '%Y-%m-%d %T') as date," . $this->db->dbprefix('sales') . ".reference_no, customer, p.paid_by, (grand_total+COALESCE(rounding, 0)), actual_shipping, w.name, companies.email as cemail")
                ->from('sales')
            ...
            $this->datatables
                ->select($this->db->dbprefix('sales') . ".id as id, DATE_FORMAT(" . $this->db->dbprefix('sales') . ".date, '%Y-%m-%d %T') as date," . $this->db->dbprefix('sales') . ".reference_no as reference_no, customer, p.paid_by, (grand_total+COALESCE(rounding, 0)), actual_shipping, w.name, companies.email as cemail")
                ->from('sales')
                ->join('warehouses w', 'w.id = sales.warehouse_id', 'left')
                ->join('companies', 'companies.id=sales.customer_id', 'left')     
                ->join('(SELECT sale_id, MAX(paid_by) as paid_by FROM sma_payments 
    ...
    public function customer_screen()
    {
        ...
        $this->load->view(
            $this->theme . 'pos/customer_screen',
            $this->data
        );
    }
    ...
    public function index($sid = NULL)
    {
        ...
        if ($this->form_validation->run() == TRUE) {
           ...
           //$this->sma->print_arrays($_POST);
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    if ($pr_item_discount == 0) {
                        $total_extra += $this->sma->formatDecimal(($item_net_price * $item_unit_quantity), 4);
                    }
                }
            } // END For Loop
            ...
            $grand_total = $this->sma->formatDecimal(($total + $total_tax + $this->sma->formatDecimal($shipping) - $this->sma->formatDecimal($return_amount) - $order_discount), 4);
            $grand_total_extra = $this->sma->formatDecimal(($total_extra - $total_with_no_points - $order_discount), 4);
            $rounding = 0;
            ...
            $data = array('date'              => $date,
                          ...
                          'return_amount'     => $this->sma->formatDecimal($return_amount),
            ...
            if (!$suspend) {
                ...
                for ($r = 0; $r < $p; $r++) {
                    ...
                    if (isset($_POST['amount'][$r]) && isset($_POST['paid_by'][$r]) && !empty($_POST['paid_by'][$r])) {
                        ...
                        } elseif($_POST['paid_by'][$r] == 'pts'){
                            $change_points += $_POST['paying_points'][$r];
                            $payment[] = array(
                                'date'         => $date,
                                // 'reference_no' => $this->site->getReference('pay'),
                                'amount'       => $amount,
            ...
            if($change_points > 0) {
                //$data['grand_total_extra'] = $data['grand_total_extra'] - $change_points;
                $data['grand_total'] = $data['grand_total'] - $change_points;
            }
    ...
    public function getProductDataByCode($code = NULL, $warehouse_id = NULL)
    {
        ...
        if ($row) {
            ...
            $pr = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'category' => $row->category_id, 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, 'no_points' => $this->sma->isPromo($row) ? 1 : $category->no_points);
    ...
    public function ajaxproducts($category_id = NULL, $brand_id = NULL)
    {
        ...
        if (!empty($products)) {
            foreach ($products as $product) {
                ...
                if (strpos($product->name, '_') !== false) {
    ...
    public function view($sale_id = NULL, $modal = NULL)
    {
        ...
        $promo = $this->pos_model->getPromotionList();
        ...
        $customer_group = $this->site->getCustomerGroupByID($this->data['customer']->customer_group_id);
        //$this->sma->print_arrays($customer_group);
        $this->data['customer']->customer_group_percent = $customer_group->percent;
    ...
    public function settings()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $data = array(
                ...
                'customers_info'            => $this->input->post('customers_info'),
                ...
                'bill_shipping'             => $this->input->post('bill_shipping'),
                'bill_return'              => $this->input->post('bill_return'),
