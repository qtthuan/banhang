...
class system_settings extends MY_Controller
{
    ...
    function track_product()
    {
        ...
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('track_product')));
        $meta = array('page_title' => lang('track_product'), 'bc' => $bc);
        $this->page_construct('settings/track_product', $meta, $this->data);
    }
    ...
    function getTrackProduct()
    {
        ...
        $this->load->library('datatables');
        $this->datatables
            ->select("id, code, max_id")
            ->from("track_product")
            //->add_column("Actions", "<div class=\"text-center\"></div>", "id");
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_track_product/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='" . lang("edit_track_product") . "'><i class=\"fa fa-edit\"></i></a></div>", "id");

        echo $this->datatables->generate();
    }

    function edit_track_product($id = NULL)
    {
        $this->form_validation->set_rules('code', lang("track_product_code"), 'trim|required');
        $track_product = $this->settings_model->getTrackProductByID($id);
        //$this->sma->print_arrays($size_group);
        if ($this->input->post('code') != $track_product->code) {
            $this->form_validation->set_rules('code', lang("track_product_code"), 'is_unique[track_product.code]');
        }
        $this->form_validation->set_rules('max_id', lang("track_product_max_id"), 'required|min_length[3]');
        ...
        } elseif ($this->input->post('edit_track_product')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/track_product");
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateTrackProduct($id, $data)) {
            $this->session->set_flashdata('message', lang("track_product_updated"));
            admin_redirect("system_settings/track_product");
        } else {
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['track_product'] = $track_product;
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_track_product', $this->data);
        }
    }
