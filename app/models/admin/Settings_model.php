...
class Settings_model extends CI_Model
{
    ...
    public function getCustomerGroupByPercent($percent)
    {
        $q = $this->db->get_where('customer_groups', array('percent' => $percent), 1);
        if ($q->num_rows() > 0) {
            return TRUE;
        }
        return FALSE;
    ...
    public function addSizeGroup($data)
    {
        if ($this->db->insert('size_group', $data)) {
            return true;
        }
        return false;
    }

    public function updateSizeGroup($id, $data = array())
    {
        $this->db->where('id', $id);
        if ($this->db->update('size_group', $data)) {
            return true;
        }
        return false;
    }

    public function getAllSizeGroup()
    {
        $q = $this->db->get('size_group');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function getSizeGroupByID($id)
    {
        $q = $this->db->get_where('size_group', array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function deleteSizeGroup($id)
    {
        if ($this->db->delete('size_group', array('id' => $id))) {
            return true;
        }
        return FALSE;
    }

    public function addPromotionList($data)
    {
        if ($this->db->insert("promotion_list", $data)) {
            return true;
        }
        return false;
    }

    public function updatePromotionList($id, $data = array())
    {
        if ($this->db->update("promotion_list", $data, array('id' => $id))) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->db->delete("promotion_list", array('id' => $id))) {
            return true;
        }
        return FALSE;
    }

    public function getPromotionListByID($id)
    {
        $q = $this->db->get_where("promotion_list", array('id' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
