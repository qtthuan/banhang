...
class Sales_model extends CI_Model
{
    ...
    public function getProductNames($term, $warehouse_id, $limit = 5)
    {
        ...
        if ($this->Settings->overselling) {
            $this->db->where("({$this->db->dbprefix('products')}.name LIKE '%" . $term . "%' OR {$this->db->dbprefix('products')}.code = '" . $term . "')");
        } else {
    ...
    public function getProductOptionsByExtraId($code, $warehouse_id, $extra_id)
    {

        $this->db->select($this->db->dbprefix('product_variants') . '.*')
            ->join('products', 'products.id=product_variants.product_id', 'left')
            ->where('product_variants.option_id_extra', $extra_id)
    ...
    public function getReturnByReferenceNo($reference_no)
    {
        $q = $this->db->get_where('sales', array('reference_no' => $reference_no, 'sale_status' => 'returned'), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
        ...
        if ($this->db->insert('sales', $data)) {
            ...
            if (!empty($si_return)) {
                ...
                foreach ($si_return as $return_item) {
                    ...
                    //$this->sma->print_arrays($data, $si_return);
            ...
            $this->sma->update_award_points($data['grand_total_extra'], $data['customer_id'], $data['created_by']);

            if ($data['sale_status'] == 'returned') {
                $this->sma->return_award_points(-$data['grand_total_extra'], $data['customer_id'], TRUE);
            }
            return true;
    ...
    public function updateSale($id, $data, $items = array())
    {
        ...
        if ($this->db->update('sales', $data, array('id' => $id)) && 
            ...
            $this->sma->update_award_points($data['grand_total_extra'], $data['customer_id'], $sale->created_by);
            return true;
    ...
    public function resetSaleActions($id, $return_id = NULL, $check_return = NULL)
    {
        if ($sale = $this->getInvoiceByID($id)) {
            ...
            if ($sale->sale_status == 'completed') {
                ...
                $this->sma->update_award_points($sale->grand_total_extra-$sale->return_amount, $sale->customer_id, $sale->created_by, TRUE);
                return $items;
            }
        }
    }
    ...
    //                         if($combo_item->type == 'standard') {
    //                             $this->site->setPurchaseItem($clause, ($qty*$combo_item->qty));
    //                         }
    //                     }
    //                 }
    //             }
