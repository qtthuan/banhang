...
class Companies_model extends CI_Model
{
     ...
     * @return bool
     */
    public function getLastBillDate($customer_id)
    {
        $this->db->select_max('date')->where('customer_id', $customer_id);
        $q = $this->db->get("sales");
        if ($q->num_rows() > 0 && $q->row()->date != null) {
            $bill_date = new DateTime($q->row()->date);
            return $bill_date->format('d-m-Y H:i:s');
        }
     ...
     * qtthuan
     * @param $id
     * @param $quest_customer
     * @return bool
     */
    public function clearBills($id, $quest_customer)
    {
    ...
    public function findCustomerSuggestions($term, $limit = 10, $lbl_award_points)
    {
        //$this->db->select("id, (CASE WHEN customer_no = '' THEN name ELSE CONCAT(name, ': ', phone, ' # " . $lbl_award_points . ": ', award_points, '  (', customer_no, ')' ) END) as text, customer_group_name, customer_group_id", FALSE);
        $this->db->select("id, (CASE WHEN customer_no = '' THEN name ELSE CONCAT(name, ': ', phone ) END) as text, name, address, phone, customer_group_name, customer_group_id", FALSE);
        $this->db->where(" (id LIKE '%" . $term . "%' OR name LIKE '%" . $term . "%' OR customer_no LIKE '%" . $term . "%' OR email LIKE '%" . $term . "%' OR phone LIKE '%" . $term . "%') ");
        $q = $this->db->get_where('companies', array('group_name' => 'customer'), $limit);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
