...
class Site extends CI_Model
{
    ...
    public function getCustomerNameID($id) {
        $q = $this->db->get_where('customer_groups', array('cusid' => $id), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getCustomerGroupByLevel($level) {
        $q = $this->db->get_where('customer_groups', array('level' => $level), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function getAllCategories() {
        $this->db->where('parent_id', NULL)->or_where('parent_id', 0)->order_by('code');
        ...
        if ($q->num_rows() > 0) {
            ...
            switch ($field) {
                case 'so':
                    ...
                    break;
                case 'pos':
                    $prefix = isset($this->Settings->sales_prefix) ? $this->Settings->sales_prefix . '' : '';
                    break;
                case 'qu':
    ...
    public function syncSalePayments($id) {
        ...
        if ($payments = $this->getSalePayments($id)) {
            ...
            if ($this->db->update('sales', array('paid' => 0, 'payment_status' => 'due'), array('id' => $id))) {
                return true;
            }
        }
    ...
    function calculateTax($product_details, $tax_details, $custom_value = NULL, $c_on = NULL) 
    {
