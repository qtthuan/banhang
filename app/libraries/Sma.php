...
class Sma
{
    ...
    {
        if ($customer != 1 && !empty($this->Settings->each_spent) && $total > 0) {
            $company = $this->site->getCompanyByID($customer);
            if ($company->customer_group_id != 1) {

                $points = round(($total / $this->Settings->each_spent) * $this->Settings->ca_point, 1);

                $total_points = $scope ? $company->award_points - $points : $company->award_points + $points;
                $total_points_per_year = $scope ? $company->points_per_year - $points : $company->points_per_year + $points;
                $this->db->update('companies',
                        array('award_points' => $total_points, 'points_per_year' => $total_points_per_year),
                        array('id' => $customer));
            }
        }
        if ($user && !empty($this->Settings->each_sale) && !$this->Customer) {
            $staff = $this->site->getUser($user);
            $points = round(($total / $this->Settings->each_sale) * $this->Settings->sa_point, 1);
    ...
    public function return_award_points($total, $customer, $scope = null)
    {
        if ($customer != 1 && !empty($this->Settings->each_spent)) {
            $company = $this->site->getCompanyByID($customer);
            if ($company->customer_group_id != 1) {
                $points = round(($total / $this->Settings->each_spent) * $this->Settings->ca_point, 1);
                //$this->sma->print_arrays($points);
                $total_points = $scope ? $company->award_points - $points : $company->award_points + $points;
                $this->db->update('companies',
                        array('award_points' => $total_points, 'points_per_year' => $company->points_per_year - $points),
                        array('id' => $customer));
            }
        }
        return true;
    ...
    public function change_award_points($points, $customer)
    {
        $company = $this->site->getCompanyByID($customer);
    ...
    public function delivery_method_opts($method = null, $empty_opt = false)
    {
        ...
        <option value="GIAO HÀNG"'.($method && $method == 'GIAO HÀNG' ? ' selected="selected"' : '').'>GIAO HÀNG</option>       
     ...
     * @return mixed|string
     */
    function getSizeNumber($option_name) {

        // Remove all spaces from option name (ex: size 1 => size1)
        $sizeNumber = preg_replace('/\s+/', '', $option_name);

        // Get size number (ex: size1 => 1, size2 => 2)
        return substr($sizeNumber, 4, strlen($sizeNumber));
    }

    /**
     * @param $from_date Ngày nhỏ 2018-12-01 22:45:00
     * @param $to_date   Ngày lớn 2019-06-02 22:45:00
     * @return number of days
     */
    function countDays($from_date, $to_date) {

        // Formulate the Difference between two dates
        return (strtotime($to_date) - strtotime($from_date))/60/60/24;
