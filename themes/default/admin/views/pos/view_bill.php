...
<head>
    ...
    <style type="text/css">
        html, body { margin: 0; padding: 0; width: 100%; height: 100%; min-width: 710px; color: #333; }
        a { outline: none; }
        h2 { margin: 20px 0; }
        .with-promo-space { }
        .bill { min-height: 600px; background: #FFF; margin: 15px 10px 10px 10px; }
        .content { display: none; }
        .with-promo-space .wrap { max-width: 55%; margin: 0 auto; padding: 0; overflow: hidden; }
        .with-promo-space .main { display: none; margin: 20px 20px 0 0; width: calc(100% - 760px); float: right; }
        .with-promo-space .content { display: block; }
        .with-promo-space .bill { width: 700px; float: left; min-height: 600px; }
        td { vertical-align: middle !important; }
        #product-list { display: block; overflow: hidden; min-height: 525px; border: 1px solid #ddd; padding: 0 10px 10px 10px; }
    ...
    <div class="bill" id="bill">
        <div id="product-list">
            <table style="margin-bottom: 0;" id="billTable" class="table table-striped table-condensed">
                <thead>
                <tr>
                    <th width="66%" class="text-center"><?= lang('product'); ?></th>
                    <th width="13%" class="text-right"><?= lang('price'); ?></th>
                    <th width="8%" class="text-right"><?= lang('qty'); ?></th>
                    <th width="14%" class="text-right"><?= lang('subtotal'); ?></th>
                </tr>
                </thead>
                <tbody></tbody>
            </table>
        ...
        <div id="totals">
            <table style="width:100%; float:right; padding:5px; color:#000; background: #FFF;" id="totalTable">
                ...
                <tr>
                    <td style="font-weight:bold;border-bottom: 1px solid #ddd;border-left: 1px solid #ddd; padding:2px 0 2px 10px; text-align:left; background-color: #909090; color: white">
                        <?= lang('total'); ?>
                    </td>
                    <td style="border-bottom: 1px solid #ddd;text-align:right; padding:2px 10px 2px 0; font-weight:bold; background-color: #909090; color: yellow"><span id="total">0.00</span></td>
                    <td style="border-bottom: 1px solid #ddd;font-weight:bold;border-left: 1px solid #fbf9f3;padding:2px 0 2px 10px; text-align:left; background-color: #909090; color: white"><?= lang('discount'); ?></td>
                    <td style="border-bottom: 1px solid #ddd;border-right: 1px solid #ddd; text-align:right; padding:2px 17px 2px 0; font-weight:bold;background-color: #909090; color: white">
                        <span id="tds">0.00</span><span id="tds_extra">0.00</span>
                    </td>
                </tr>
                <tr>
                    <td style="border-bottom: 1px solid #ddd;border-left: 1px solid #ddd; padding:2px 0 2px 10px; text-align:left; font-weight:bold; background-color: #909090; color: white">
                        <?= lang('shipping'); ?>
                    </td>
                    <td style="border-bottom: 1px solid #ddd;text-align:right; padding:2px 10px 2px 0; font-weight:bold; background-color: #909090; color: white"><span id="tship">0.00</span>
                    </td>
                    <td style="border-bottom: 1px solid #ddd;font-weight:bold;border-left: 1px solid #fbf9f3;padding:2px 0 2px 10px; text-align:left; background-color: #909090; color: white"><?= lang("return_amount"); ?></td>
                    <td style="border-bottom: 1px solid #ddd;border-right: 1px solid #ddd; text-align:right; padding:2px 17px 2px 0; font-weight:bold;background-color: #909090; color: white">
                        <span id="treturn" style="font-weight: bold">0.00</span></td>
                </tr>
                ...
                <tr>
                    ...
                    <td style="border-bottom: 1px solid #ddd;text-align:right; padding:2px 10px 2px 0; font-weight:bold; background-color: #909090; color: white"><span id="titems">0</span></td>
                    <td style="border-bottom: 1px solid #ddd;padding: 2px 0px 5px 10px; text-align:left; font-size: 1.3em; border-left: 1px solid #ddd; font-weight:bold; background:#ff2222; color:#FFF;"><?= lang('total_payable'); ?></td>
                    <td
                        style="border-bottom: 1px solid #ddd;text-align:right; padding:2px 17px 5px 0px; font-size: 1.5em; font-weight:bold; background:#ff2222; color:#FFF;">
...
<script type="text/javascript">
    var product_tax = 0, invoice_tax = 0, total_discount = 0, product_discount = 0, order_discount = 0, total = 0, tax_rates = <?php echo json_encode($tax_rates); ?>, pos_customer = localStorage.getItem('poscustomer'), shipping = parseFloat(localStorage.getItem('posshipping'));
    ...
    function loadItems() {
        ...
        if (localStorage.getItem('positems')) {
            ...
            $.each(sortedItems, function () {
                ...
                tr_html += '<td class="text-right">'+formatQuantity2(item_qty) + '</td>';
            ...
            var preturn = localStorage.getItem('posreturn');
            var pcustomer = localStorage.getItem('poscustomer');
            ...
            var gtotal = total;
            if (order_discount > 0) {
                gtotal = gtotal - order_discount;
            }
            if (preturn > 0) {
                gtotal = gtotal - preturn;
            }
            if (shipping > 0) {
            ...
            $('#treturn').text(parseFloat(preturn) > 0 ? '-' + formatMoney(preturn) : '0');
            if (site.settings.tax2 != 0) {
                $('#ttax2').text(formatMoney(invoice_tax));
            }
            $('#tship').text(parseFloat(shipping) > 0 ? formatMoney(shipping) : '0');
...
<script>
    ...
    setInterval(function() { shipping = parseFloat(localStorage.getItem('posshipping')); loadItems() }, 2000);
</script>
</body>
</html>
