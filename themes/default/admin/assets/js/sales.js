...
$('#slreturn').focus(function () {
    ...
    if (!is_numeric($(this).val())) {
        ...
        returns = $(this).val() ? parseFloat($(this).val()) : '0';
    }
    localStorage.setItem('slreturn', returns);
    var gtotal = ((total + invoice_tax) - order_discount - returns) + shipping;
    $('#gtotal').text(formatMoney(gtotal));
    $('#treturn').text(formatMoney(returns));
});
if (slreturn = localStorage.getItem('slreturn')) {
    returns = parseFloat(slreturn);
    $('#slreturn').val(returns);
} else {
    returns = 0;
}
    ...
    $('#reset').click(function (e) {
        bootbox.confirm(lang.r_u_sure, function (result) {
            if (result) {
                ...
                if (localStorage.getItem('slreturn')) {
                    localStorage.removeItem('slreturn');
                }
        ...
        $.ajax({
            ...
            success: function (data) {
                if(data.result === 'success') {
                    slitems[mid] = {"id": mid, "item_id": mid, "label": gcname + ' (' + gccode + ')', "row": {"id": mid, "code": gccode, "name": gcname, "quantity": 1, "base_quantity": 1, "price": gcprice, "real_unit_price": gcprice, "tax_rate": 0, "qty": 1, "type": "manual", "discount": "0", "serial": "", "option":""}, "tax_rate": false, "options":false};
                    localStorage.setItem('slitems', JSON.stringify(slitems));
                    loadItems();
                    $('#gcModal').modal('hide');
                    $('#gccard_no').val('');
    ...
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function () {            
            ...
            tr_html = '<td>' +
                        ...
                        '<span class="sname" id="name_' + row_no + '">' + i + '. ' + item_code + item_name +(sel_opt != '' ? ' ('+sel_opt+')' : '')+'</span>' + 
                        (item_comment ? '<br /><span class="scomment" style="font-style: italic;"> (' + item_comment + ')  </span>' : '') +
                        (item_comment_name ? '<span class="scomment" style="font-style: italic;"> - ' + item_comment_name + '  </span>' : '') +
                        '<i class="pull-right fa fa-edit tip pointer edit" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer; font-size: 38px;"></i>' +
                     '</td>';
        ...
        var gtotal = parseFloat(((total + invoice_tax) - order_discount - returns) + shipping);
